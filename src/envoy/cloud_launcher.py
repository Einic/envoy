...
def main():
    ...
    Console.debug_b(
        '''
            Envoy-Cloud - A web interface to ChatGPT
            Github: https://github.com/Einic
    ...
    try:
        from envoy_cloud.server import ChatBot as CloudServer

        return CloudServer(args.proxy, args.verbose, args.sentry, True).run(args.server, args.threads)
    except (ImportError, ModuleNotFoundError):
        Console.error_bh('### You need `pip install Envoy-ChatGPT[cloud]` to support cloud mode.')
